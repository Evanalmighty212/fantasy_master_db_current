...
class TestSevenStatusesExhaustiveAndExclusive:
    def test_all_seven_statuses_reachable_and_valid(self):
        players = pd.DataFrame(columns=["gsis_id", "position", "draft_year", "draft_round", "draft_pick", "rookie_season"])
        history = pd.DataFrame(columns=["season", "player_id", "games_played", "fantasy_points_ppr"])
        lookup = _ep_lookup(
        ...
        rows_and_kwargs[5] = (rows_and_kwargs[5][0], {**rows_and_kwargs[5][1], "players_df": players_dm})
